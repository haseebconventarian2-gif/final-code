...
async def generate_text(user_prompt: str, system_prompt: str | None = None, rag_context: str | None = None) -> str:
    ...
    messages.append(
      {
        "role": "system",
        "content": (
          "RAG context (primary source of truth):\n"
          f"{rag_context}"
        ),
      }
    )
  ...
  body = {
    "messages": messages + [{"role": "user", "content": str(user_prompt or "")}],
    "temperature": 0.3,
  }
