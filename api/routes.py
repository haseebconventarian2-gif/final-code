...
async def send_text(to_e164: str, text: str) -> None:
  ...
  # IMPORTANT: your installed SDK requires the 'body' parameter
    ...
    for event in events:
      ...
      if event_type == "Microsoft.Communication.AdvancedMessageReceived":
        ...
        if audio_path:
          ...
          print("AUDIO reply bytes:", len(audio_out))
          if sender:
            try:
              await send_text(sender, answer)
              print("ACS SEND OK ->", sender)
            except Exception as e:
              print("ACS SEND FAILED ->", repr(e))
          else:
            print("ACS SEND SKIPPED: sender missing")
          return JSONResponse({"ok": True, "mode": "debug-audio"})
        print("RAG CHECK input:", text)
        answer = await get_answer(text or "")
        print("RAG CHECK answer:", answer)
        if sender:
          try:
            await send_text(sender, answer)
            print("ACS SEND OK ->", sender)
